...
(userController.signIn = async (req, res, next) => {
  try {
    ...
    db.query(idsFromDbQuery).then((data) => {
      ...
      if (data.rowCount == 1) {
        ...
        db.query(insertQuery)
          .then((data) => {
            console.log(
              'User added to the SQL Database. Row count:  ',
              data.rowCount
            );
  ...
  (userController.myQ = async (req, res, next) => {
    try {
      ...
      const myQ = await db.query(
        `SELECT * FROM questions WHERE googleid=${googleID}`
      );
      res.locals.myQ = myQ;
      return next();
    } catch (err) {
  ...
  (userController.newestTen = async (req, res, next) => {
    try {
      const newestTen = await db.query(
        'SELECT * FROM questions ORDER BY date DESC LIMIT 10'
      );
...
userController.AddQuestion = async (req, res, next) => {
  try {
    ...
    db.query(insertQuery)
      ...
      .catch((err) =>
        console.log('error while inserting question to DB: ', err)
      );
  } catch (err) {
    const defaultErr = {
...
userController.searchByCompany = async (req, res, next) => {
  try {
    ...
    db.query(searchQuery)
      ...
      .catch((err) =>
        console.log('error while searching for items by company name: ', err)
      );
  } catch (err) {
    const defaultErr = {
...
userController.searchByType = async (req, res, next) => {
  try {
    ...
    db.query(searchQuery)
      ...
      .catch((err) =>
        console.log('error while searching for items by question type: ', err)
      );
  } catch (err) {
    const defaultErr = {
...
userController.searchByRole = async (req, res, next) => {
  try {
    ...
    db.query(searchQuery)
      ...
      .catch((err) =>
        console.log('error while searching for items by role: ', err)
      );
  } catch (err) {
    const defaultErr = {
